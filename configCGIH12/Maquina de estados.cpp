...
void Animation() {
	...
	if (AnimDog)
	{
		rotDog -= 0.6f;
		//printf("%f", rotBall);
		float tiempo = glfwGetTime();	//variable de control, siempre aumenta
		FLegs = sin(tiempo * 2.5f) * 20.0f;	//patas frontales, seno (-1 - 1)
		RLegs = cos(tiempo * 2.5f) * 20.0f;	//patas traseras cos desfasado a las delanteras
		dogPos.z += 0.005f;	//avanza el perro
		head = sin(tiempo * 1.0f) * 10.0f;	//mueve la cabeza
		tail = sin(tiempo * 3.0f) * 30.0f; //cola
	}
