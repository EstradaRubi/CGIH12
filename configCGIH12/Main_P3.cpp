...
int main() {
 ...
 //     
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// --- CUBO 1 
		model = glm::translate(model, glm::vec3(4.0f, -1.0f, 2.0f)); //Izquierda
		model = glm::rotate(model, 0.5f, glm::vec3(1.0f, 0.0f, 0.0f));;// Rotación constante en Y
		model = glm::scale(model, glm::vec3(2.5f, 2.5f, 2.5f)); // Grande
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);

		// --- CUBO 2
		model = glm::translate(model, glm::vec3(0.0f, 2.0f, -2.0f)); // Arriba y un poco al fondo
		model = glm::rotate(model, glm::radians(45.0f), glm::vec3(1.0f, 0.0f, 1.0f)); // Inclinación fija en X y Z
		model = glm::scale(model, glm::vec3(0.8f, 0.8f, 0.8f)); // Pequeño
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);

		//// --- CUBO 3
		//model = glm::translate(model3, glm::vec3(4.0f, -1.0f, 2.0f)); // Derecha, abajo y al frente
		//model = glm::rotate(model3, (float)glfwGetTime() * 2.0f, glm::vec3(0.0f, 0.0f, 1.0f)); // Rotación rápida en Z
		//model = glm::scale(model3, glm::vec3(0.5f, 3.0f, 0.5f)); // Estirado verticalmente (forma de pilar)
		//glDrawArrays(GL_TRIANGLES, 0, 36);
